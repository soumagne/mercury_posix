...
#include <mercury_util/mercury_atomic.h>

static hg_atomic_int32_t hg_posix_finalizing_count_g;
...
getcwd_cb(hg_handle_t handle)
{
    ...
    /* Call function */
    MERCURY_GEN_LOG_MESSAGE("hg_posix_getcwd");
    ...
    /* Respond back */
    hg_ret = HG_Respond(handle, NULL, NULL, &out_struct);
    if (hg_ret != HG_SUCCESS) {
 ...
 */
#ifndef HG_POSIX_HAS_OPEN64
MERCURY_GEN_CALLBACK_STUB(open_cb, hg_posix_open,
...
#else
MERCURY_GEN_CALLBACK_STUB(open64_cb, hg_posix_open64,
        MERCURY_GEN_TRUE, hg_int32_t,
        MERCURY_GEN_TRUE, open_in_t, open_in_params,
        MERCURY_GEN_FALSE, open_out_t, ,
        MERCURY_GEN_FALSE, ,
...
readdir_cb(hg_handle_t handle)
{
    ...
    /* Call function */
    MERCURY_GEN_LOG_MESSAGE("hg_posix_readdir");
...
#define MERCURY_POSIX_REGISTER_SEQ(r, data, elem) \
    MERCURY_POSIX_REGISTER(elem);

static void
register_posix(void)
{
    BOOST_PP_SEQ_FOR_EACH(MERCURY_POSIX_REGISTER_SEQ, ,
            REGISTER_SEQ LARGE_FILE_REGISTER_SEQ);

    MERCURY_REGISTER(HG_CLASS_DEFAULT, "hg_posix_getcwd", getcwd_in_t, getcwd_out_t, getcwd_cb);
#ifndef HG_POSIX_HAS_OPEN64
    MERCURY_REGISTER(HG_CLASS_DEFAULT, "hg_posix_open", open_in_t, open_out_t, open_cb);
#else
    MERCURY_REGISTER(HG_CLASS_DEFAULT, "hg_posix_open64", open_in_t, open_out_t, open64_cb);
#endif
    MERCURY_REGISTER(HG_CLASS_DEFAULT, "hg_posix_readdir", readdir_in_t, readdir_out_t, readdir_cb);
    MERCURY_REGISTER(HG_CLASS_DEFAULT, "hg_posix_sync", void, void, sync_cb);
}
...
main(int argc, char *argv[])
{
    ...
    HG_Hl_init(NULL, HG_TRUE);

    /* Initalize atomic variable to finalize server */
    hg_atomic_set32(&hg_posix_finalizing_count_g, 0);
    ...
    do {
        ...
        do {
            hg_ret = HG_Trigger(HG_CLASS_DEFAULT, HG_CONTEXT_DEFAULT, 0, 1,
                    &actual_count);
        } while ((hg_ret == HG_SUCCESS) && actual_count);

        if (hg_atomic_cas32(&hg_posix_finalizing_count_g, 1, 1))
            break;

        hg_ret = HG_Progress(HG_CLASS_DEFAULT, HG_CONTEXT_DEFAULT, HG_MAX_IDLE_TIME);
